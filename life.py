...
class App:
    ...
    RD_PRESETS = [
        ("Spots (α)", "Circular spots that fill space", 0.035, 0.065),
        ("Stripes", "Labyrinthine stripe patterns", 0.025, 0.060),
        ("Coral Growth", "Branching coral-like tendrils", 0.055, 0.062),
        ("Mitosis", "Self-replicating spots that divide", 0.0367, 0.0649),
        ("Worms", "Moving worm-like solitons", 0.078, 0.061),
        ("Waves", "Pulsating concentric wave patterns", 0.014, 0.054),
    ]
    ...
    # Generalises Game of Life: continuous states, smooth ring kernels, and a
    # growth function that maps local potential to cell delta.
    ...
    PHYSARUM_PRESETS = [
        # (name, description, sensor_angle, sensor_dist, turn_speed, move_speed, deposit, decay, num_agents_ratio)
        ("Explorer", "Sparse network — long-range foraging", 0.4, 9.0, 0.3, 1.0, 0.5, 0.02, 0.15),
        ("Dense Web", "Thick interconnected veins", 0.3, 5.0, 0.4, 1.0, 0.8, 0.01, 0.30),
        ("Tendrils", "Thin branching filaments", 0.6, 12.0, 0.2, 1.5, 0.3, 0.03, 0.10),
        ("Pulsing", "Rhythmic contraction patterns", 0.35, 7.0, 0.5, 0.8, 1.0, 0.04, 0.20),
        ("Maze Solver", "Finds shortest paths between food", 0.25, 8.0, 0.6, 1.0, 0.6, 0.015, 0.25),
        ("Galaxy", "Spiral arm formation", 0.8, 10.0, 0.15, 1.2, 0.4, 0.025, 0.12),
    ]
    ...
    BOIDS_PRESETS = [
        # (name, description, num_ratio, sep_r, ali_r, coh_r, sep_w, ali_w, coh_w, max_spd)
        ("Murmuration", "Tight flocks with predator avoidance", 0.08, 2.5, 7.0, 12.0, 2.5, 1.2, 1.0, 1.2),
        ("Fish School", "Slow, cohesive groups", 0.06, 3.0, 6.0, 10.0, 1.5, 1.5, 1.8, 0.6),
        ("Swarm", "Fast, loosely coupled agents", 0.10, 2.0, 10.0, 15.0, 1.0, 0.8, 0.5, 1.5),
        ("Migration", "Long-range aligned movement", 0.04, 3.0, 15.0, 20.0, 1.2, 2.0, 0.8, 1.0),
        ("Dense Flock", "Tightly packed formation", 0.12, 2.0, 5.0, 8.0, 2.0, 1.5, 2.0, 0.8),
        ("Chaos", "High separation, low cohesion", 0.10, 4.0, 6.0, 8.0, 3.0, 0.5, 0.3, 1.8),
    ]

    # Direction arrows: 8 directions based on velocity angle
    BOIDS_ARROWS = ["↑", "↗", "→", "↘", "↓", "↙", "←", "↖"]
    ...
    ACO_PRESETS = [
        # (name, description, evaporation, deposit_strength, diffusion, num_ants_ratio, num_food, food_radius)
        ("Forager", "Basic foraging with moderate trails", 0.02, 0.3, 0.01, 0.08, 4, 3),
        ("Highway", "Strong trails — ants form highways", 0.005, 0.6, 0.005, 0.12, 3, 4),
        ("Explorer", "High evaporation — ants spread widely", 0.05, 0.2, 0.02, 0.10, 6, 2),
        ("Swarm", "Many ants, quick convergence", 0.015, 0.4, 0.01, 0.25, 5, 3),
        ("Minimal", "Few ants, slow trail build-up", 0.01, 0.5, 0.008, 0.04, 2, 5),
        ("Feast", "Abundant food, dense pheromone clouds", 0.03, 0.35, 0.015, 0.15, 8, 4),
    ]
    ACO_DENSITY = ["  ", "░░", "▒▒", "▓▓", "██"]
    ...
    MAZE_PRESETS = [
        # (name, description, gen_algo, solve_algo, maze_scale)
        # maze_scale: 1=full density, 2=half (bigger corridors visually)
        ("Classic DFS + A*", "Recursive backtracker maze solved with A*", "backtracker", "astar", 1),
        ("Prim + Dijkstra", "Prim's random maze with Dijkstra solver", "prim", "dijkstra", 1),
        ("Kruskal + BFS", "Kruskal's maze with breadth-first search", "kruskal", "bfs", 1),
        ("Backtracker + DFS", "DFS generation and DFS solving", "backtracker", "dfs", 1),
        ("Prim + A*", "Prim's algorithm with A* pathfinding", "prim", "astar", 1),
        ("Kruskal + Dijkstra", "Kruskal's random maze with Dijkstra", "kruskal", "dijkstra", 1),
    ]
    ...
    RPS_PRESETS = [
        ("Classic Spiral Waves",
         "3 species, balanced — beautiful self-organizing spirals",
         3, 0.5, "random"),
        ("Slow Spirals",
         "Lower swap rate — larger, slower spirals",
         3, 0.2, "random"),
        ("Fast Chaos",
         "High swap rate — turbulent rapid dynamics",
         3, 0.9, "random"),
        ("Territorial Blocks",
         "Species start in large blocks — watch invasion fronts",
         3, 0.5, "blocks"),
        ("Five Species",
         "Rock-Paper-Scissors-Lizard-Spock — 5 cyclic species",
         5, 0.5, "random"),
        ("Seeded Spirals",
         "Small clusters — spiral nucleation from seeds",
         3, 0.4, "seeds"),
    ]
            ...
            for col in range(view_cols):
                ...
                if av < 0.05 and cv < 0.05:
                    ...
                    if cv > 0.15:
                        ...
                        attr = curses.color_pair(2) | curses.A_DIM  # dim green
                else:
                    # Very faint
                    ch = "··"
                    attr = curses.color_pair(4) | curses.A_DIM
            ...
            for col in range(view_cols):
                ...
                if view == "bacteria":
                    ...
                    if bv > 0.7:
                        ...
                        ch = "▓▓"
                        attr = curses.color_pair(2) | curses.A_BOLD  # bright green
                    elif bv > 0.2:
                        ch = "▒▒"
                        attr = curses.color_pair(2)  # green
                    elif bv > 0.08:
                        ch = "░░"
                        attr = curses.color_pair(2) | curses.A_DIM  # dim green — frontier
                    elif bv > 0.02:
                        ch = "··"
                        attr = curses.color_pair(2) | curses.A_DIM
                    elif nv < 0.3:
